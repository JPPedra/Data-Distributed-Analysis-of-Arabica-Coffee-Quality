...
write.csv(coffee_new, file = "Arabica-coffee-half-modified.csv", row.names = FALSE)

# Will load the results from the bag weight function
```
...
bags_threshold # returns 1062, which is the outlier
...
The plot shows no outliers. However, the frequency for level 0 of quakers is extremly high. 
...
Furthermore, it is crutial to investigate which processing method is used the most and the least - with the countries that produce more coffee (as previously investigated before)
