...
## 3.5 Identifying and Dealing with Outliers

This section investigates whether the current dataset has any outliers - any identified outliers were corrected, when possible.

The investigation plan starts by analysing the structure of the dataset. Followed by investigating the distribution of each variable to draw possible conclusions about their outliers.
...
Having analysed the structure of the current dataset; investigate each variable separately, starting by Country.of.Origin. This to understand how each variable is distributed.
...
# plot to investigate any outliers - displaying the distribution of the variable makes it possible to identify any extreme values that correspond to possible outliers.
...
The boxplot shows that the variable has one possible outlier -  there seems to be one observation higher than all other observations; corresponds to the maximum value of this variable. 
...
The summary function output shows a Max of 1062 - according to the histogram plotted above, this makes sense to be the outlier identified.

Moreover, it is then necessary to identify the problem with 1062 number of bags, as it is a value out of the ordinary for this variable and is considered to be a possible outlier - boxplot function is used below to confirm if the value is indeed an outlier.
...
The boxplot with min functions return the value of 1062 as the possible outlier.
...
Therefore, this coffee beans have less quality than others and that is why there are more number of bags for line 735 - which makes this instance not an outlier - it does not need further investigation.
...
ggplot(coffee_new_1, aes(Number.of.Bags)) + geom_boxplot(color="black",fill="gold") + theme_minimal() + ggtitle("Quakers in the coffee quality")
```
...
Finally, function count shows how both variables range between 0 and 10, which is excpected - there are no outliers.
...
Most coffee beans have a 0.1 level of moisture levels - count function output shhows that the previous identified extreme values are not outliers.
...
The histogram and count function show that the variable is not normal distributed. Hence, this variable should probably not be used in the machine learning method as it would disrupt the accuracy of the model.
...
ggplot(aes(
  x = n,
  y = reorder(
    coffee_new_1$Country.of.Origin,
    coffee_new_1$Processing.Method
  )
)) + geom_bar(stat = "identity", fill = "#0072B2") + labs(x = "Coffee Produced", y = "Country of Origin") + theme(
...
```{r}
ggplot(coffee_new_1, aes(x=Processing.Method, y=Total.Cup.Points, fill=Processing.Method)) +
  geom_boxplot() +
  theme_classic() +
  coord_flip()
```
...
#   Total.Cup.Points >= 85 ~ 'Very good',
#   Total.Cup.Points >= 80 ~ 'Good',
#   Total.Cup.Points < 80 ~ 'Bad'
# ))
#
